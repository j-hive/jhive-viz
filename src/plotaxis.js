...
export function initializePlotAxis() {
    ...
    plotAxisContainers.svg = d3.select("#plot-decorators")
        .append("svg")
    ...
    plotAxisContainers.yAxis = plotAxisContainers.svg.append("g")
        .attr("class", "y-axis")
        .attr("transform", `translate(${plottingConfig.LEFTMARGIN},0)`)
        .call(d3.axisLeft(windowState.yScaler)
            .tickSizeOuter(0)
            .tickSize(-windowState.WIDTH * 1.3)
        );

    plotAxisContainers.xAxis = plotAxisContainers.svg.append("g")
        .attr("class", "x-axis")
        .attr("transform", `translate(0,${windowState.HEIGHT - plottingConfig.LEFTMARGIN})`)
        .call(d3.axisBottom(windowState.xScaler)
            .tickSizeOuter(0)
            .tickSize(-windowState.HEIGHT * 1.3));


    // Add Axes Labels
    plotAxisContainers.yLabel = plotAxisContainers.svg.append("text")
        .attr("class", "y-label")
    ...
    plotAxisContainers.xLabel = plotAxisContainers.svg.append("text")
        .attr("class", "x-label")
        .attr("text-anchor", "middle")
...
export function scalePlotAxis(zoomedXScaler, zoomedYScaler) {
    ...
    plotAxisContainers.xAxis.call(
        d3.axisBottom(zoomedXScaler).tickSizeOuter(0).tickSize(-windowState.HEIGHT * 1.3)
    )

    plotAxisContainers.yAxis.call(
        d3.axisLeft(zoomedYScaler).tickSizeOuter(0).tickSize(-windowState.WIDTH * 1.3)
    );
...
export function transformXAxis(zoomedXScaler) {
    plotAxisContainers.xAxis.transition()
        .duration(1000)
        .call(d3.axisBottom(zoomedXScaler).tickSizeOuter(0).tickSize(-windowState.HEIGHT * 1.3));
}

export function transformYAxis(zoomedYScaler) {
    plotAxisContainers.yAxis.transition()
        .duration(1000)
        .call(d3.axisLeft(zoomedYScaler).tickSizeOuter(0).tickSize(-windowState.HEIGHT * 1.3));
}

// Setting Axis Labels

export function setYLabel(text) {
    plotAxisContainers.yLabel.text(
        text
    )
}

export function setXLabel(text) {
    plotAxisContainers.xLabel.text(
        text
    )
}
...
export function resizePlotAxis() {
    plotAxisContainers.svg.attr("width", windowState.WIDTH).attr("height", windowState.HEIGHT)
}