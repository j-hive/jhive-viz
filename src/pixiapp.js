...
export async function initializePixiApp() {
    ...
    windowState.xRange = getRangeWithBorder(dataContainers.metadata[windowState.currentXAxis]);
    windowState.yRange = getRangeWithBorder(dataContainers.metadata[windowState.currentYAxis]).reverse();


    // Setting up Initial Scalers 

    windowState.xScaler = d3.scaleLinear()
        .domain(windowState.xRange)
        .range([0, windowState.WIDTH]);

    windowState.yScaler = d3.scaleLinear()
    ...
    function zoomed(transform) {
        ...
        const zoomed_x_scaler = transform.rescaleX(windowState.xScaler).interpolate(d3.interpolateRound);
        const zoomed_y_scaler = transform.rescaleY(windowState.yScaler).interpolate(d3.interpolateRound);
    ...
    function switch_x_axis() {
        ...
        const zoomed_x_scaler = windowState.currentZoom.rescaleX(windowState.xScaler).interpolate(d3.interpolateRound);
    ...
    function switch_y_axis() {
        ...
        const zoomed_y_scaler = windowState.currentZoom.rescaleY(windowState.yScaler).interpolate(d3.interpolateRound);
